...
void XorBlock(
  const BYTE* const in, const BYTE* out
) // Ensure that this is always 32 bit aligned
{
  uint_fast8_t i;

  for (i = 0; i < AES_BLOCK_WORDS; i++) ((DWORD*)out)[i] ^= ((DWORD*)in)[i];
}
...
void MixColumnsR(BYTE* __restrict__ state) {
  ...
  for (; i < AES_BLOCK_WORDS; i++) {
    ...
    ((DWORD*)state
    )[i] = MulE(word) ^ ROR32(MulB(word), 8) ^ ROR32(MulD(word), 16) ^ ROR32(Mul9(word), 24);
  }
}
...
void AesInitKey(AesCtx* Ctx, std::vector<byte> key) {
  ...
  for (i = (uint_fast8_t)RijndaelKeyDwords; i < (Ctx->rounds + 1) << 2; i++) {
    ...
    if ((i % RijndaelKeyDwords) == 0) {
      temp =
        std::byteswap(SubDword(ROR32(std::byteswap(temp), 24)) ^ RCon[i / RijndaelKeyDwords]);
    }
...
static void ShiftRows(BYTE* state) {
  ...
  for (i = 0; i < AES_BLOCK_BYTES; i++) state[i] = bIn[(i + ((i & 3) << 2)) & 0xf];
}
...
static void SubBytesR(BYTE* block) {
  ...
  for (i = 0; i < AES_BLOCK_BYTES; i++) block[i] = GetSBoxR(block[i]);
}

void AesEncryptCbc(
  const AesCtx* const Ctx, BYTE* __restrict__ data, size_t* __restrict__ len
) {
