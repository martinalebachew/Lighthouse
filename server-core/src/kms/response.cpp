...
static const BYTE DefaultHwId[8] = {0x3A, 0x1C, 0x04, 0x96,
                                    0x00, 0xB6, 0x00, 0x76};
...
Response::Response(KMS::Request &request) {
  ...
  byte pidexample[] = {
      0x35, 0x00, 0x35, 0x00, 0x30, 0x00, 0x34, 0x00, 0x31, 0x00, 0x2d,
      0x00, 0x30, 0x00, 0x30, 0x00, 0x32, 0x00, 0x30, 0x00, 0x36, 0x00,
      0x2d, 0x00, 0x35, 0x00, 0x35, 0x00, 0x35, 0x00, 0x2d, 0x00, 0x36,
      0x00, 0x35, 0x00, 0x37, 0x00, 0x35, 0x00, 0x33, 0x00, 0x30, 0x00,
      0x2d, 0x00, 0x30, 0x00, 0x33, 0x00, 0x2d, 0x00, 0x36, 0x00, 0x31,
      0x00, 0x35, 0x00, 0x36, 0x00, 0x2d, 0x00, 0x37, 0x00, 0x36, 0x00,
      0x30, 0x00, 0x31, 0x00, 0x2e, 0x00, 0x30, 0x00, 0x30, 0x00, 0x30,
      0x00, 0x30, 0x00, 0x2d, 0x00, 0x32, 0x00, 0x31, 0x00, 0x33, 0x00,
      0x32, 0x00, 0x30, 0x00, 0x31, 0x00, 0x39, 0x00, 0x00, 0x00};
...
std::vector<byte> Response::toEncryptedBuffer() {
  ...
  memcpy(buffer.data() + offsetof(Response, KmsPID) + PIDSize, &CMID,
         sizeof(Response) - offsetof(Response, CMID));

  // Calculate the size of the encrypted data in the buffer and the required
  // padding
  size_t encryptSize = sizeof(Response) - offsetof(Response, IV) -
                       sizeof(KmsPID) + PIDSize; // Number of bytes to encrypt
