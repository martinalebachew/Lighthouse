...
BindAck::~BindAck() { free(p_result_list.p_results); }

std::vector<byte> BindAck::toBuffer() {
  // Allocate buffer with size of the fixed-size properties
  std::vector<byte> buffer = std::vector<byte>(offsetof(BindAck, sec_addr) +
                                               offsetof(port_any_t, port_spec));
  ...
  unsigned int offset =
      offsetof(BindAck, sec_addr) + offsetof(port_any_t, port_spec);
