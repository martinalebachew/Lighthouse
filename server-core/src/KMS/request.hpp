...
struct Request {
  ...
  */
  ...
  DWORD LicenseStatus;     // 0 = Unlicensed, 1 = Licensed (Activated), 2 = OOB
                           // grace, 3 = OOT grace, 4 = NonGenuineGrace, 5 =
                           // Notification, 6 = extended grace
  DWORD BindingExpiration; // Expiration of the current status in minutes (e.g.
                           // when KMS activation or OOB grace expires)
  GUID AppID;              // Can currently be Windows, Office2010 or Office2013
  GUID ActID; // aka SKUID. Most detailed product list. One product key per
              // ActID. Ignored by KMS server
  GUID KMSID; // This is what the KMS server uses to grant or refuse activation
  GUID CMID;  // Client machine Id. Used by the KMS server for counting minimum
              // clients
  ...
  WCHAR WorkstationName[64]; // Workstation name. FQDN if available, NetBIOS
                             // otherwise
  ...
  inline Request(std::vector<byte> stub) {
    ...
    if (stub.size() != sizeof(Request))
      throw std::runtime_error(
          "Invalid stub size. Expected " + std::to_string(sizeof(Request)) +
          " bytes, got " + std::to_string(stub.size()) + " bytes.");
