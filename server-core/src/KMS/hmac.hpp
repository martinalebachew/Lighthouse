...
int_fast8_t Sha256Hmac(BYTE *key, BYTE *__restrict__ data, DWORD len,
                       BYTE *__restrict__ hmac) {
  Sha256HmacCtx Ctx;
 ...
 */
static int_fast8_t CreateV6Hmac(BYTE *const encrypt_start,
                                const size_t encryptSize,
                                const int_fast8_t tolerance) {
  BYTE hash[32];
  const uint8_t halfHashSize = sizeof(hash) >> 1;
  BYTE *responseEnd = encrypt_start + encryptSize;

  constexpr size_t V6_POST_EPID_SIZE =
      124; // sizeof(KMS::Response) - offsetof(KMS::Response, CMID);

  // This is the time from the response
  FILETIME *ft =
      (FILETIME *)(responseEnd - V6_POST_EPID_SIZE +
                   sizeof(GUID)); // sizeof(((KMS::Response*)0)->CMID)
  ...
  uint64_t timeSlot =
      (GET_UA64LE(ft) / TIME_C1 * TIME_C2 + TIME_C3) + (tolerance * TIME_C1);
  ...
  if (!Sha256Hmac(
          ...
          (DWORD)(encryptSize -
                  16), // encryptSize minus the HMAC itself, 16 bytes
          hash // use same buffer for resulting hash where the key came from
          )) {
    return false;
  }

  memcpy(responseEnd - 16, hash + halfHashSize,
         halfHashSize); // HMAC size is 16 bytes
  return true;
}
