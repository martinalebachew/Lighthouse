...
void XorBlock(const BYTE *const in,
              const BYTE *out) // Ensure that this is always 32 bit aligned
{
  uint_fast8_t i;
...
void MixColumnsR(BYTE *__restrict__ state) {
  ...
  for (; i < AES_BLOCK_WORDS; i++) {
    ...
    ((DWORD *)state)[i] = MulE(word) ^ ROR32(MulB(word), 8) ^
                          ROR32(MulD(word), 16) ^ ROR32(Mul9(word), 24);
  }
}
...
void AesInitKey(AesCtx *Ctx, std::vector<BYTE> key) {
  ...
  static const DWORD RCon[] = {0x00000000, 0x01000000, 0x02000000, 0x04000000,
  ...
  for (i = (uint_fast8_t)RijndaelKeyDwords; i < (Ctx->rounds + 1) << 2; i++) {
    ...
    if ((i % RijndaelKeyDwords) == 0)
      temp = std::byteswap(SubDword(ROR32(std::byteswap(temp), 24)) ^
                           RCon[i / RijndaelKeyDwords]);
...
static void MixColumns(BYTE *state) {
  ...
  for (; i < AES_BLOCK_WORDS; i++) {
    DWORD word = ((DWORD *)state)[i];
    ((DWORD *)state)[i] =
        Mul2(word) ^ ROR32(Mul3(word), 8) ^ ROR32(word, 16) ^ ROR32(word, 24);
  }
}
