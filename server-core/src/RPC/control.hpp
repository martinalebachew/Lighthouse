...
namespace RPC::Control {
    struct Conversation {
        std::vector<byte> inline ReceiveFragment(boost::asio::ip::tcp::socket &socket) {
            ...
            boost::asio::read(socket, boost::asio::buffer(fragment.data() + HEAD_SIZE * sizeof(fragment[0]),
        ...
        void inline SendFragment(boost::asio::ip::tcp::socket &socket, std::vector<byte> fragment) {
            boost::asio::write(socket, boost::asio::buffer(fragment));
        }

        void inline SendPDU(boost::asio::ip::tcp::socket &socket, std::vector<byte> pdu) {
            // No support for fragmented PDUs
