...
std::vector<byte> NDR_32_V2_TRANSFER_SYNTAX = {0x04, 0x5d, 0x88, 0x8a, 0xeb, 0x1c, 0xc9, 0x11, 0x9f, 0xe8, 0x08, 0x00, 0x2b, 0x10, 0x48, 0x60, 0x02, 0x00, 0x00, 0x00};
...
struct BindAck {
    ...
    */
    ...
    BindAck(Bind& bind, u_int16 port) :
        ...
        if (!assoc_group_id) { // If the value is 0, the client is asking for a new association group
        ...
        // TODO: implement proper support for multiple transfer syntaxes. for now, assuming one transfer syntax per context element
    ...
    std::vector<byte> toBuffer() {
        std::vector<byte> buffer = std::vector<byte>(offsetof(BindAck, sec_addr) + offsetof(port_any_t, port_spec)); // Allocate buffer with size of the fixed-size properties
        ...
        buffer.resize(buffer.size() + alignmentSize); // Resize the buffer to fit the alignment
        offset += alignmentSize;

        buffer.resize(buffer.size() + offsetof(p_result_list_t, p_results)); // Resize the buffer to fit the fixed-size properties of p_result_list_t
        memcpy(buffer.data() + offset, // Copy into the buffer
